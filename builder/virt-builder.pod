...
Note that newlines in the index file are not significant.  Blank lines
between sections are optional and are only included for readability.
You may use any number of blank lines (or none at all) to separate
sections.
